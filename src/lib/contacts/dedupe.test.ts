...
describe('shouldUseWhatsappName', () => {
  it('fills a blank or phone-shaped fallback', () => {
    expect(shouldUseWhatsappName(null, '+55 13 99999-9999', 'João Silva')).toBe(
      true
    );
    expect(
      shouldUseWhatsappName('5513999999999', '+55 13 99999-9999', 'João Silva')
    ).toBe(true);
  });

  it('never overwrites a real CRM name', () => {
    expect(
      shouldUseWhatsappName('João do CRM', '+55 13 99999-9999', 'João WhatsApp')
    ).toBe(false);
  });
});
