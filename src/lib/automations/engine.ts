...
async function runStep(
  ...
  switch (step.step_type) {
    case 'sales_qualify': {
      const cfg = step.step_config as SalesQualifyStepConfig;
      if (cfg.mode !== 'suggestion') throw new Error('sales_qualify only supports suggestion mode');
      if (!args.contactId || !args.context.message_text) throw new Error('sales_qualify needs contact + inbound message');
      const { data: contact, error: contactError } = await db.from('contacts').select('qualification_score, main_pain').eq('id', args.contactId).eq('account_id', args.automation.account_id).maybeSingle();
      if (contactError) throw new Error(`sales_qualify contact lookup failed: ${contactError.message}`);
      if (!contact) throw new Error('sales_qualify contact not found in account');
      const currentScore = Number(contact?.qualification_score ?? 0);
      const result = classifySalesIntent(String(args.context.message_text), currentScore);
      const nextScore = Math.max(0, Math.min(100, currentScore + result.scoreDelta));
      const { error: suggestionError } = await db.from('automation_sales_suggestions').insert({
        account_id: args.automation.account_id, automation_id: args.automation.id,
        contact_id: args.contactId, conversation_id: args.context.conversation_id ?? null,
        source_message: String(args.context.message_text), intent: result.intent,
        confidence: result.confidence, score_delta: result.scoreDelta,
        temperature: result.temperature, pain: result.pain,
        suggested_reply: result.suggestedReply, next_action: result.nextAction,
        human_handoff: result.humanHandoff,
      });
      if (suggestionError) throw new Error(`sales_qualify suggestion failed: ${suggestionError.message}`);
      const patch: Record<string, unknown> = { qualification_score: nextScore, lead_temperature: result.temperature, last_sales_intent: result.intent, next_best_action: result.nextAction };
      if (result.pain && !contact?.main_pain) patch.main_pain = result.pain;
      const { error: updateError } = await db.from('contacts').update(patch).eq('id', args.contactId).eq('account_id', args.automation.account_id);
      if (updateError) throw new Error(`sales_qualify contact update failed: ${updateError.message}`);
      return `intent ${result.intent}; score ${currentScore}->${nextScore}; suggestion created${result.humanHandoff ? '; human handoff recommended' : ''}`;
    }
    case 'send_message': {
