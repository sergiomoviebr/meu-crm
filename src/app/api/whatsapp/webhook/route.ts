...
import { attributeLeadIfNeeded, referralFromMetaWebhook } from '@/lib/traffic/attribution';
...
interface WhatsAppMessage {
  ...
  context?: { id: string };
  /**
   * Present on the message that opened the chat when the customer
   * came from a click-to-WhatsApp ad or Facebook/Instagram post.
   * `source_id` is the ad/post's external id (matched against
   * `ads.external_id` for attribution — src/lib/traffic/attribution.ts);
   * `ctwa_clid` is the click id Meta uses for its own ads reporting.
   */
  referral?: {
    source_url?: string;
    source_id?: string;
...
export async function GET(request: Request) {
  try {
    ...
    logger.error('Webhook GET verification failed', {
...
async function processMessage(
  ...
  // Click-to-WhatsApp ad attribution — best-effort, never blocks the
  // message insert below. See src/lib/traffic/attribution.ts.
  if (isFirstInboundMessage && message.referral) {
    try {
      await attributeLeadIfNeeded(supabaseAdmin(), {
        accountId,
        contactId: contactRecord.id,
        isFirstInboundMessage: true,
        referral: referralFromMetaWebhook(message.referral),
      });
    } catch (err) {
      logger.error('Failed to attribute lead from ad referral', {
        operation: 'webhook.attributeLead',
        accountId,
...
async function findOrCreateContact(
  ...
  if (existingContact) {
    ...
    if (shouldUseWhatsappName(existingContact.name, phone, name)) {
      updates.name = name.trim();
    }
    if (!existingContact.whatsapp) updates.whatsapp = phone;
    if (!existingContact.source) updates.source = 'WhatsApp';
    if (!existingContact.relationship_type) updates.relationship_type = 'lead';
    ...
    return {
      contact: { ...existingContact, ...updates },
      wasCreated: false,
    };
  }
