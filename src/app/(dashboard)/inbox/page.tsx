...
function InboxPageInner() {
  ...
  const handleConversationEvent = useCallback(
    (event: {
      ...
      if (event.eventType === 'DELETE') {
        // Realtime DELETE payloads only reliably carry the primary key in
        // `old` (replica identity default), which is all we need here.
        const deletedId = event.old?.id;
        if (!deletedId) return;
        setConversations((prev) => prev.filter((c) => c.id !== deletedId));
        knownConvIdsRef.current.delete(deletedId);
        // Covers deletion from another tab/teammate — the deleting tab
        // itself already clears its own selection via
        // onConversationDeleted, ahead of this round-trip.
  ...
  const handleConversationOpened = useCallback(
    (conversation: Conversation) => {
      knownConvIdsRef.current.add(conversation.id);
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        >
          <MessageThread
            ...
            onConversationDeleted={handleCloseConversation}
          />
        </div>
