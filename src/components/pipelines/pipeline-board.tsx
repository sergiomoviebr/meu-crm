...
interface PipelineBoardProps {
  ...
  onPreviewDeal: (deal: Deal) => void;
  onRemindDeal: (deal: Deal) => void;
        ...
        {sortedStages.map((stage) => {
          ...
          return (
            <StageColumn
              ...
              onPreviewDeal={onPreviewDeal}
              onRemindDeal={onRemindDeal}
              settings={settings}
              now={now}
            />
          );
...
function StageColumn({
  ...
  const awaitingCount = deals.filter(
  ...
  return (
    ...
    <div className="border-border bg-card/60 flex w-[85vw] max-w-[320px] min-w-[260px] shrink-0 snap-start flex-col rounded-xl border p-4 lg:w-auto lg:max-w-none lg:flex-1 lg:shrink lg:basis-[260px] lg:snap-none">
      ...
      {awaitingCount > 0 && (
        <p className="mt-1 flex items-center gap-1 text-[11px] font-medium text-red-600 dark:text-red-300">
        ...
        {deals.length === 0 ? (
          ...
          deals.map((deal) => (
            <DraggableDealCard
              ...
              stage={stage}
              onEdit={onEditDeal}
              onPreview={onPreviewDeal}
              onRemind={onRemindDeal}
              settings={settings}
              now={now}
            />
          ))
